...
const MedTracker = () => {
    ...
    const handleTake = (indexToRemove) => {
        const updatedMedicines = medicines.filter((_, index) => index !== indexToRemove);
        setMedicines(updatedMedicines);
        localStorage.setItem("medicines", JSON.stringify(updatedMedicines));
    };

    const handleEdit = (index) => {
        navigate("/reminders", { state: { editIndex: index } });
    };
    ...
    return (
        <div className="min-h-screen p-6 -ml-[260px] flex flex-col items-center">
            ...
            <div className="ml-[210px] mt-4 flex gap-24">
                {days.map((day) => (
                    <button
                        ...
                        style={{ color: "black" }}
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="65" viewBox="0 0 31 31" fill="none">
            ...
            {medicines.filter(med => !selectedDay || med.day === selectedDay).length === 0 ? (
                <p className="mr-[550px]">No medicines inputted for this day.</p>
            ) : (
                medicines
                    .filter(med => !selectedDay || med.day === selectedDay)
                    .map((med, index) => (
                        <div key={index} className="w-[800px] h-[300px] ml-12 mb-6 flex-shrink-0 rounded-[10px] border border-black/25 p-6">
                            <div className="flex flex-col">
                                <div className="flex flex-row items-center justify-between">
                                    ...
                                    <div className="w-[90px] h-[40px] pl-1 py-1 flex-shrink-0 rounded-[8px] bg-blue text-white font-bold text-lg flex items-center justify-center">
                                        {new Date(med.date).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    className="w-[90px] h-[40px] pl-1 py-1 flex-shrink-0 rounded-[8px] bg-blue text-white font-bold text-lg flex items-center justify-center cursor-pointer"
                                >
                                    Take
                                </div>
                                <div
                                    onClick={() => handleEdit(index)}
                                    className="w-[90px] h-[40px] ml-[660px] -mt-10 pl-1 py-1 flex-shrink-0 rounded-[8px] bg-[#FF7F7F] text-white font-bold text-lg flex items-center justify-center cursor-pointer"
                                >
                                    Edit
                                </div>
                            </div>
